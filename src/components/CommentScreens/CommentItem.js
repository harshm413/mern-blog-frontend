...
const CommentItem = ({ comment, activeUser }) => {
        ...
        const getCommentLikeStatus = async () => {
            ...
            try {
                const { data } = await axios.post(
                    `https://mern-blog-backend-nc2o.onrender.com/comment/${comment_id}/getCommentLikeStatus`,
                    { activeUser },
                    {
                        headers: {
    ...
    const handleCommentLike = async () => {
        ...
        try {
            const { data } = await axios.post(
                `https://mern-blog-backend-nc2o.onrender.com/comment/${comment_id}/like`,
                { activeUser },
                {
                    headers: {
                        'Content-Type': 'application/json',
                    ...
                    {[...Array(5)].map((_, index) => {
                        return (
                            <FaStar
                                ...
                                size={15}
                                color={
                                    comment.star > index ? '#0205b1' : 'grey'
                                }
                            />
                        );
                    })}
